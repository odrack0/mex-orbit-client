...
| `ALT_HERO` / `ALT_PLAYER` / `ALT_NPC_BAND` | `game/entity_node.gd` (`entity.json` → `altitude`) | 80 · 50 · 40 | **Altura de vuelo** (3-sep, el z-index del 3D): el héroe por encima de todo, las otras naves en medio y cada NPC en una altura estable de su banda (por id); dos cuerpos en el mismo punto se apilan en vez de atravesarse. La cámara enfoca al héroe a su altura (sigue centrado); clic, HUD, números de daño, rayo y explosión se proyectan a la altura de cada entidad. La posición lógica no cambia. El original iba todo a y=0 (naves planas) |
...
- **`PortalNode`** (`game/portal_node.gd`): su malla **de pie**, como el jumpgate del DO 3D
  ...
  **el centro** (el vórtice), que **en reposo no está**, solo aparece si `encendido.vortice` es
  ...
  tres rondas en vivo hasta quedar limpio); `PortalNode` busca el hijo
...
`_volar_a` (`game/world.gd`) clampea a `[-RADIACION_ALCANCE, _limites + RADIACION_ALCANCE]`
(50 000 u), el **mismo número** que `Dials.RadiationReach` del server — cliente y
autoridad siguen coincidiendo en el destino. No es una pared: la radiación es un
reloj y la nave sigue **hasta explotar**; el tope está donde nadie llega con vida.
(Un primer margen de 1000 u se sintió como un muro a un paso del límite.) Por el lado del 0 eso es **negativo**:
...
|---|---|---|---|---|
| `render` | 0,65× | 0,85× | 1× | **la palanca de GPU**: fill-rate del skybox, nebulosas, partículas. El 2D no escala. Se amplía con **FSR**, no bilineal: el bilineal dejaba las siluetas contra el negro con escalones de 2 px («la nave se ve pixelada», reportado en vivo). Calibrado en vivo con FSR puesto: MEDIA de 0,75 a 0,85 y BAJA de 0,5 a 0,65 — la nitidez se nota y el coste apenas |
